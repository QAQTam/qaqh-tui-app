...
PROBE_DELAY=${PROBE_DELAY:-25}   # 杀 daemon 后等多久按 Ctrl+R（须 > STALL_AFTER=15s）
...
QAQH_DATA_DIR="$D/qaqh" timeout 65 script -qec "stty rows 40 cols 130; timeout 60 $TUI" /dev/null \
  <"$FIFO" > "$D/tui.raw" 2>&1 &
TPID=$!
sleep 8;  echo "t=8   杀 daemon#1"; kill -9 "$D1" 2>/dev/null; rm -f "$D/qaqh/daemon.json"
sleep 24; echo "t=32  起 daemon#2"; D2=$(start); wait_pid "$D2" && echo "t=32  daemon#2 pid=$D2 epoch=$(epoch)"
