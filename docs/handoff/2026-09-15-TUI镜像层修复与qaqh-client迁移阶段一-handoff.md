...
> ### ⚠ 本文档状态：**阶段一 / 1.5 / 二均已完成**（2026-09-15 当日收尾）
>
> 下文 §2/§4/§5 记录的是**开工前的评估与前置判断**，保留作为推理留存；其中若干结论已被
> 实际施工推翻或消化，逐条对账如下：
>
> | 本文档的判断 | 实际结果 |
> |---|---|
> | 「阶段二被后端 2 个枚举变体阻塞」（§2.4/§4/§5.1） | **已解除**：变体由后端 `a72ce0c` 补上 |
> | 「9 处 `.service(`」（§2.5） | 实测 **8 处**；验收判据 `rg "\.service\(" src/` 现已**零命中** |
> | 「`http.rs` 不可能降到 0，修正目标 ≈700 行」（§2.5 验收） | **推翻**：阶段 1.5 后 `transport/` 整个目录删除，`http.rs` = **0 行** |
> | 「`reqwest` 必须留到 1.5」（§2.5 步骤 5） | 已随 `http.rs` 一并删除 |
> | 「双份凭据来源」风险（§2.5 风险） | 阶段 1.5 后**只剩一份**（qaqh-client 内部） |
>
> 实际落点（`wc -l` 实测）：`protocol/` 2481 行（9 文件）→ **405 行**（3 文件：
> `mod.rs` 71 / `session_meta.rs` 128 / `snapshot.rs` 206，且只剩 TUI 自有视图，
> **无任何 wire 镜像**）；`transport/` 目录删除；`runtime.rs` 949 → **431** 行。
> 迁移期间新发现并登记的缺陷：T-09～T-13（详见 buglist）。
...
| 项 | 说明 |
|---|---|
| ~~T-01 本体~~ | **已完成**：阶段一（`1eca0b3` 前后）、阶段二（`a278ab8`/`1eca0b3`/`868c744`/`b23be65`/`b1a150f`）、阶段 1.5（`d379d90`） |
...
| ~~后端 2 个枚举变体~~ | **已解除**：后端 `a72ce0c` 已补 `QueryRequest::{SessionDashboard, TodoStatus}` |
...
1. ~~给后端提那 2 个枚举变体~~ → **已完成**（后端 `a72ce0c`）。
2. ~~T-01 阶段一开工~~ → **已开工并完成**，连同阶段 1.5 与阶段二。
3. **唯一剩下的硬阻塞是后端 `BUG-2026-09-15-02`**（daemon 启动期工具探测可无限挂起）：
   真机端到端一条未跑（daemon 重启自愈 / 租约过期自愈 / `Lagged` 终止帧恢复 / 多标签+子代理
   并发 / `Ctrl+R` 重连）全部被它堵住。**这不是 TUI 侧能绕过的问题。**
