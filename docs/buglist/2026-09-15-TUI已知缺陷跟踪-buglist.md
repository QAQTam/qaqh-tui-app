...
| 2026-09-15 | **「与 daemon 失联」误报修复（后端 `BUG-2026-09-15-07`）**：机主报告「后端一直在工作且前端一直在输出，TUI 却一直提示失联」。根因不在本仓也不在 daemon，而在 **客户端没有把 keepalive 当作存活信号**：daemon 每 15s 发一次，`sse_decoder.rs` 把它当注释行丢掉，而 `ChannelStatus::Open` 只在建连那一刻发一次——于是 `last_open` 量的是「连接建立了多久」，任何活过阈值的连接都被判失联，且因 keepalive 让空闲重连永不发生而**永不自愈**。修法：客户端新增 `on_liveness`（每消费一个 chunk 回调一次，解码**之前**），本仓接线并撤掉上一版 60s 的妥协、回到 `STALL_AFTER=20s`。后端 `d9fa81c`、本仓 `2ff0351`。**首轮归因（「越活跃越像失联」）是错的**，被机主一句「daemon 怎么会忙？」问倒后重查——那条教训写进了后端条目 |
