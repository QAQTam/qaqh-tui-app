...
> 日期：2026-09-21（§10 于 2026-09-23 增补：性能门禁已落地并接入 CI）
> 状态：进行中（首帧/流式/长会话/commit 基准已建；**门禁已自动化**）
...
## 10. 性能门禁（2026-09-23 新增）

### 10.1 为什么之前不算「门禁」

§1–§7 的基准都是 `#[ignore]` 的手工测量：**退化不会让任何东西变红**。M6.4 的
验收项写的是「性能、内存与迁移**门禁**」，所以本轮把判据变成可执行的。

### 10.2 只钉结构性性质，不钉绝对耗时

§0 已注明「绝对耗时会随 CPU powersave/boost 状态波动」——本轮复测也印证了：
同一台机器上 `render` 从 529ms 漂到 841ms（+59%），`replay` 从 104ms 漂到
138ms。**拿绝对时间当门禁只会得到一条随机变红的 CI**。

所以门禁钉的是「一旦退化到某个量级就必然是 bug」的性质，阈值同时满足两条：
离实测有 1~2 个数量级余量，离**被修复过的回归形态**也有 1 个数量级：

| 判据 | 实测（2026-09-23） | 阈值 | 被修复过的回归形态 |
|---|---:|---|---|
| version 未变同步 | 0.85–1.3 µs | **< 1 ms** | 整历史重扫 ~27.7 ms/帧 |
| live delta / 帧 | 9.9–11 µs | **< 2 ms** | `sync_start` 取「第一个已知 turn」→ 3.79 ms/帧 |
| 440 回合 runtime 常驻 | 542 KB | **< 8 MB** | 每 block 存完整 seed/turn/block 字符串 |
| 440 / 110 块数比 | 3.996 | **3.5–4.5** | 有界队列把长 replay 截断（4096 上限）→ 比值塌陷 |
| 440 回合 replay | 129–139 ms | **< 3 s** | 超线性扫描 |

### 10.3 入口

```bash
scripts/perf-gate.sh            # 硬门禁（CI 用，约 0.3s）
scripts/perf-gate.sh --full     # 硬门禁 + 完整基线记录（写 $OUT，默认 /tmp）
```

**必须 `--test-threads=1`**：内存判据走全局分配器计数，`cargo test` 默认并行会让
其它测试的分配污染 `live()`。这不是可选项，脚本里写死了。

已接进 `scripts/ci-linux.sh`（`cargo test` 之后、`clippy` 之前），所以 CI 里
**退化会直接打红**。
...
- 把 `sync_start` 改回「第一个已知 turn」（历史回归形态）→
  `live delta 必须微秒级：实测 3.794701ms/帧（阈值 2ms）` → **FAIL**（已还原）；
- 顺带发现：单独拆掉 version 门**不会**让门禁变红（no-op 同步降到 86µs 仍在
  1ms 内），因为 `sync_start` 的高水位短路还在。也就是说两条防线是**冗余**的，
  真正抓回归的是 delta 那条——这一点值得记下来，别以为拆了 version 门就没人管。

### 10.5 本轮基线（2026-09-23 复测）

```text
=== M6.4 性能门禁（20 tools/回合）===
version 未变同步: 932ns（阈值 <1ms）
live delta/帧:   11.146µs（阈值 <2ms）
440 回合常驻:    542 KB / 10123 blocks（阈值 <8MB）
440 回合 replay: 138.5387ms（阈值 <3s）
块数线性度:      3.996（阈值 3.5–4.5）

=== 规模曲线 ===
 turns   blocks    replay ms live delta µs       render ms  steady live
    25      578         7.84             9           46.24        37 KB
    50     1153        14.09            10           97.32        70 KB
   110     2533        31.07            10          208.36       138 KB
   220     5063        63.49            11          435.26       274 KB
   440    10123       128.50            16          841.28       545 KB

=== v1 对照（流式刷新成本，110 回合历史）===
① delta 流式 ×20: avg 810µs/帧
② tick 帧动 ×40: avg 728µs/帧
③ 热路径重入（50 工具活动回合）: 970µs
```
