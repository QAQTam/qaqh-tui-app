...
`selftest` 是**不触网**的纯逻辑用例（19 项）：增量 id 比较、投递过滤、`parse_since`、
渲染截断标注、游标往返与损坏退化。存在理由：本工具的失败模式大多是
**静默漏消息**而非报错，必须有可证伪的用例钉住边界。
...
- **评论编辑的检测是线程级的**：只有「线程 updated_at 变了但没有新评论」时才会报
  `*_comment_edited`，不区分是哪一条被编辑、也不给 diff。要看内容直接读 inbox 前后两条。
- **`issues list-issues` 不接受 `--state all`**（实测 400），实现里改成 open/closed
  各拉一次再合并；PR 接口则接受 `--state all`。
