...
`selftest` 是**不触网**的纯逻辑用例（37 项；复核：`python3 cnb-chat/cnb_chat.py selftest | grep -cE '^  (ok|FAIL)'`
...
| 去掉 `idle >= REFETCH_ROUNDS` 兜底（回到「删+加且计数不变就漏」） | 4 项红 |
| 成功拉取后不重置 `idle`（兜底退化成每轮都拉） | 1 项红 |
...
- **「删一条 + 加一条」且计数与 `updated_at` 都不变时，新评论/评审最迟 `REFETCH_ROUNDS` 轮补投**：
  游标（`last_comment_id` / `last_review_id`）用 `max(...)` **单调推进、从不回退**，而 `dirty`
  只看计数与 `updated_at` —— 这种组合下评论面本来**根本不会重拉**（2026-09-20 打桩复现：
  连续三轮 `events=[]`，新评论永远看不到）。**兜底**：每线程连续 `REFETCH_ROUNDS` 轮（默认 10）
  没成功拉过该面，就无条件重拉一次；成功后计数归零。
  代价：每线程每 N 轮多一次请求（默认 20s/轮 ⇒ 约 200s 一次）；最坏情况下事件**延后**最多
  N 轮。是否真会触发取决于平台删除评论时是否推进 `updated_at`（**未实测**）。
