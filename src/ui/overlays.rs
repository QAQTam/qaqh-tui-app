...
pub fn draw(f: &mut Frame, app: &App, area: Rect) {
    let Some(top) = app.overlays.last() else {
        return;
    };
    match top {
        Overlay::SessionList {
...
fn draw_session_list(f: &mut Frame, app: &App, area: Rect, selected: usize, show_archived: bool) {
    ...
    for (idx, m) in items.iter().enumerate() {
        ...
        let mut spans = vec![
            Span::styled(
                format!(" {flag} "),
                if open { theme::accent() } else { theme::dim() },
            ),
            Span::styled(
                format!("{title:<36}"),
                if selected_now {
...
fn draw_help(f: &mut Frame, area: Rect) {
    ...
    let keys: Vec<(&str, &str)> = vec![
        ...
        (
            "Ctrl+T",
            "新建会话（回退链：QAQH_DEFAULT_CWD>启动目录>当前会话）",
        ),
        (
            "/ + Tab/↑↓/Enter",
        ...
        (
            "/new [cwd]",
            "留空静默用回退链，/new ? 或 /new Tab 打开二级 cwd 编辑（支持 ~/）",
        ),
...
fn draw_cwd(f: &mut Frame, input: &[char], cursor: usize, area: Rect) {
    let inner = box_frame(
        ...
        80,
        5,
        "新建会话 · cwd（绝对路径，留空走 QAQH_DEFAULT_CWD>启动目录>当前会话）Enter 确认 · Esc 取消",
    );
    let cursor = cursor.min(input.len());
    let before: String = input[..cursor].iter().collect();
    let before_w = before.width() as u16;
