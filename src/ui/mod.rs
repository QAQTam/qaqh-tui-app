...
/// 根布局切分（`draw` 与渲染缓存键共用）。
pub fn root_areas(area: Rect, composer_height: u16) -> RootAreas {
    let [tab, main, composer, status] = Layout::vertical([
        Constraint::Length(1),
        Constraint::Min(3),
...
#[cfg(test)]
pub fn transcript_content_width(area: Rect, composer_height: u16, show_workspace: bool) -> u16 {
    transcript_viewport(area, composer_height, show_workspace).0
}
...
/// （历史上 A1 的宽度键错位就是这么来的）。
pub fn transcript_viewport(area: Rect, composer_height: u16, show_workspace: bool) -> (u16, usize) {
    let root = root_areas(area, composer_height);
...
pub fn draw(f: &mut Frame, app: &App) {
    let area = f.area();
    let root = root_areas(area, composer::height(app));
                ...
                for composer_h in [3u16, 8] {
                    ...
                    terminal
                        .draw(|f| {
                            ...
                            let key = transcript_content_width(area, composer_h, show_ws);
                            // ② draw 内部取法（ui::draw → transcript::draw）。
                            let root = root_areas(area, composer_h);
