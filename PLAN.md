> ⚠ **本文档不是本仓（qaqh-tui-app）的计划。** 它是 `qaqh-webui` 仓的重写计划，随本仓
> 初始化（`757353b`，2026-08-30「first version」）一并带入，此后从未替换。
> 本仓自己的计划在 `docs/plan/`；本文仅作历史留档。§2/§4 的协议契约内容与后端一致，
> 可作参照，但**不是本仓的权威来源**。
> 是否移入 `docs/archive/` 或直接删除，留待所有者决策（issue #22 · W-01）。
